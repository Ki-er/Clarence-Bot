...
async function getPagesLength (url){
    ...
    let result = 1
    const number = dom.window.document.getElementsByClassName('next_page').item(0)?.previousElementSibling?.innerHTML
...
exports.getQuote = async (tag) => {
    ...
    let quoteTexts = quoteItem.firstChild.wholeText.trim().slice(1, -1)
    let quoteAuthor =  quoteItem?.childNodes.item(3)?.textContent?.trim().split(',')[0].trim()
