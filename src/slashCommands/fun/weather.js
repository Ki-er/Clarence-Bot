        ...
        weather.getAllWeather(function(err, JSONObj){
            if (err != null){
                ...
                const embed = new MessageEmbed()
                    .setColor('ORANGE')
                    .setFooter({
                        text: `Speed of Wind : ${parseInt(JSONObj.wind.speed)} m/s · Humidity: ${JSONObj.main.humidity}% · Pressure: ${JSONObj.main.pressure}hPa `,
                    })
                    .setTitle(`Weather of ${city}`)
                    .setDescription(
                        `${parseInt(JSONObj.main.temp)} °C ${ChangeFirstLetterToUpperCase(JSONObj.weather[0].description)}`
                    )
                    .setThumbnail(`http://openweathermap.org/img/wn/${JSONObj.weather[0].icon}@2x.png`);
