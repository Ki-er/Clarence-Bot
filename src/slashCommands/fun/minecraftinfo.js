...
module.exports = {
	...new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName('block').setDescription('Name of block').setRequired(true)
		),
	/**
	 *
	 * @param {Client} client
	...
	run: async (client, interaction) => {
		...
		if (blockInfo) {
			...
			textureUrl =
				'https://raw.githubusercontent.com/rom1504/minecraft-assets/master/data/1.18.1/' +
				mcAssets.getTexture(blockInfo.name) +
				'.png';
			textureUrl = textureUrl.replace('minecraft:block', 'blocks');
			await axios({
				// find out if texture is available
				method: 'get',
				url: textureUrl,
			})
				.then((response) => {
			...
			const embed = new MessageEmbed()
				...
				.setAuthor({
					...
					iconURL: isTextureAvailable ? textureUrl : null,
				})
				.setTimestamp()
				.addFields(
					...
					{
						name: 'Diggable',
						value: blockInfo.diggable ? 'Yes' : 'No',
						inline: true,
					},
